...
//hallo FK
        ...
        while (rsData.next()) {
            ...
            for (int i = 1; i <= meta.getColumnCount(); i++) {
                ...
                if (val == null) sbInsert.append("NULL");
                else sbInsert.append("'").append(val.toString().replace("'", "''")).append("'");
                if (i < meta.getColumnCount()) sbInsert.append(", ");
            }
            if (!pkFields.isEmpty()) {
            ...
            for (String pkPostgres : pkValuesPostgres) {
                if (!pkValuesInterbase.contains(pkPostgres)) {
                    ...
                    for (int i = 0; i < pkFields.size(); i++) {
                        ...
                        if (i < pkFields.size() - 1) where.append(" AND ");
                    }
